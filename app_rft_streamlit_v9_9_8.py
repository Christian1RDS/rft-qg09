...
st.set_page_config(page_title='RFT Automático - V9.9.8', page_icon='R', layout='wide')
DB='rft_v998_local.db'
POSTOS=['QG09','QG07']
...
FALHA_CAND=['FALHA','DEFEITO','DS_DEFEITO','NM_DEFEITO','TIPO_FALHA','DESCRICAO_DEFEITO','DESCRIÇÃO_DEFEITO','DESC_DEFEITO','DS_FALHA','NM_FALHA','CAUSA','PROBLEMA']
...
def load(c, posto=None, ano=None):
    ...
    if not df.empty:
        df['dt']=pd.to_datetime(df['dt'],errors='coerce'); df['dpu']=pd.to_numeric(df['dpu'],errors='coerce').fillna(0); df['falha']=df['falha'].fillna('').astype(str).str.strip(); df=df[df['dt'].notna()].copy()
    return df
...
def read_file(up):
    ...
    if ext=='csv':
        ...
        if b[:2] in (b'\xff\xfe', b'\xfe\xff'):
            attempts += [('utf-16',';'),('utf-16','\t'),('utf-16',','),('utf-16',None)]
        attempts += [('utf-8-sig',None),('utf-8-sig',';'),('latin1',';'),('latin1',','),('utf-16',';'),('utf-16',None)]
        last=None
        for enc,sep in attempts:
            try:
                return clean(pd.read_csv(io.BytesIO(b), encoding=enc, sep=sep, engine='python' if sep is None else None))
            except Exception as e: last=e
        raise ValueError(last)
    raise ValueError('Use .xlsx, .xls ou .csv')

def clean(df):
    d=df.copy(); d.columns=[str(c).strip().replace('\ufeff','').replace('ÿþ','') for c in d.columns]; return d
...
def detect_falha(cols):
    ...
    for c in cols:
        u=str(c).upper()
        if 'FALHA' in u or 'DEFEITO' in u: return c
    return None
...
def save_upload(c,name,df):
    cur=c.execute('INSERT INTO uploads (file_name,uploaded_at,rows,message) VALUES (?,?,?,?)',(name,datetime.now().isoformat(timespec='seconds'),len(df),'Upload V9.9.8'))
...
def plot_svg(p,posto,ano,ini,fim):
    ...
    poly=' '.join([f'{x:.1f},{y:.1f}' for x,y,_ in pts]); parts.append(f"<polyline points='{poly}' fill='none' stroke='#111827' stroke-width='3'/>")
...
c=con()
st.markdown("<div class='hero'><div style='font-size:1.75rem;font-weight:900;'>RFT Automático - V9.9.8</div><div class='small'>Versão separada: CSV UTF-16 corrigido, sem Plotly/Matplotlib, Pareto clássico em SVG e Dashboard completo.</div></div>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if df.empty: st.info('Sem dados para o posto/ano selecionado.')
        ...
        cols=st.columns(5); vals=[('RFT Diário',daily),('RFT Semanal',weekly),('RFT Mensal',monthly),('RFT Anual',annual),('RFT YTD',ytd)]
        for col,(name,val) in zip(cols,vals): col.metric(name,fmt(val[0]),f'Total: {val[1]}')
...
with tabs[2]:
    st.subheader('Base & Upload'); st.warning('Mapeie as colunas. CSV UTF-16 com cabeçalho ÿþ foi tratado nesta versão.'); modo_imp=st.radio('Modo de importação',['Somar ao histórico','Substituir período sobreposto','Reprocessar o ano inteiro'],horizontal=True); up=st.file_uploader('Base operacional (.xlsx, .xls ou .csv)',type=['xlsx','xls','csv']); prepared=None
    if up is not None:
        try:
...
with tabs[4]:
    st.subheader('Sobre'); st.write('V9.9.8: versão separada, sem Plotly/Matplotlib, com correção de CSV UTF-16, Pareto clássico em SVG e Dashboard com Diário/Semanal/Mensal/Anual.')
