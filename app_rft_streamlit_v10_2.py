...
st.set_page_config(page_title="RFT Automático - V10.2", page_icon="R", layout="wide")
DB="rft_v102_local.db"
POSTOS=["QG09","QG07"]
CANON=["NR_WO","DT_HR_INSPECAO","C_DPU_QG_AMARELO","CD_POSTO_CN"]
ALIASES={"NR_WO":["NR_WO","WO","ORDEM","OP"],"DT_HR_INSPECAO":["DT_HR_INSPECAO","DATA","DATA_INSPECAO","DATA_HORA","DT_HR"],"C_DPU_QG_AMARELO":["C_DPU_QG_AMARELO","DPU","QTD_DEFEITO","QTD_DEFEITOS","QUANTIDADE","QTD"],"CD_POSTO_CN":["CD_POSTO_CN","POSTO","ESTACAO","ESTAÇÃO","CD_POSTO"],"CD_MODELO":["CD_MODELO","MODELO","MODEL","PRODUTO"]}
...
def save_upload(c,name,df):
    cur=c.execute("INSERT INTO uploads (file_name,uploaded_at,rows,message) VALUES (?,?,?,?)",(name,datetime.now().isoformat(timespec='seconds'),len(df),'Upload V10.2'))
...
# ---------- App ----------
c=con(); st.markdown("<div class='hero'><div style='font-size:1.75rem;font-weight:900;'>RFT Automático - V10.2</div><div class='small'>Cards RFT corrigidos: abaixo da meta = vermelho | acima ou igual à meta = verde. Sem setas automáticas do Streamlit.</div></div>", unsafe_allow_html=True)
        ...
        if dfp.empty: st.info('Sem falhas preenchidas para o posto/ano selecionado.')
            ...
            if ini>fim: st.error('A data inicial não pode ser maior que a data final.')
                ...
                if not p.empty:
                    ...
                    escolha=st.selectbox('Escolha uma falha do Top para estratificar por modelo',p['Falha'].tolist()); detalhe=filt[filt['falha']==escolha].copy(); detalhe['modelo']=detalhe['modelo'].replace('','Sem modelo'); modelos=detalhe['modelo'].value_counts().head(20).reset_index(); modelos.columns=['Modelo','Quantidade']; st.markdown(f"### Estratificação por modelo — {escolha}"); st.bar_chart(modelos.set_index('Modelo'),use_container_width=True); st.dataframe(modelos,use_container_width=True,hide_index=True)
with tabs[3]:
...
with tabs[5]: st.subheader('Sobre'); st.write('V10.2: substitui st.metric por cards próprios para aplicar a regra abaixo da meta = vermelho | acima ou igual à meta = verde, sem setas automáticas.')
