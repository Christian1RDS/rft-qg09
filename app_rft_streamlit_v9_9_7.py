...
st.set_page_config(page_title="RFT Automático - V9.9.7", page_icon="R", layout="wide")

DB = "rft_v997_local.db"
POSTOS = ["QG09", "QG07"]
CANON = ["NR_WO", "DT_HR_INSPECAO", "C_DPU_QG_AMARELO", "CD_POSTO_CN"]
FALHA_CANDIDATES = ["FALHA", "DEFEITO", "DS_DEFEITO", "NM_DEFEITO", "TIPO_FALHA", "DESCRICAO_DEFEITO", "DESCRIÇÃO_DEFEITO", "DESC_DEFEITO", "NOME_FALHA", "DS_FALHA", "NM_FALHA", "TIPO_DEFEITO", "CAUSA", "DESCRICAO", "DESCRIÇÃO", "OBS_DEFEITO", "PROBLEMA"]
ALIASES = {
...
def init_db(conn):
    conn.execute("CREATE TABLE IF NOT EXISTS uploads (id INTEGER PRIMARY KEY AUTOINCREMENT, file_name TEXT, uploaded_at TEXT, rows INTEGER, message TEXT)")
    conn.execute("CREATE TABLE IF NOT EXISTS dados (id INTEGER PRIMARY KEY AUTOINCREMENT, upload_id INTEGER, nr_wo TEXT, dt TEXT, dpu REAL, posto TEXT, falha TEXT)")
    conn.commit()
...
def create_upload(conn, file_name, rows, message="Upload V9.9.7"):
    cur = conn.execute("INSERT INTO uploads (file_name, uploaded_at, rows, message) VALUES (?, ?, ?, ?)", (file_name, datetime.now().isoformat(timespec="seconds"), int(rows), message))
    conn.commit()
...
def insert_rows(conn, upload_id, df):
    ...
    for r in df.itertuples(index=False):
        rows.append((int(upload_id), str(r.NR_WO), r.DT_HR_INSPECAO.isoformat(sep=" ", timespec="seconds"), float(r.C_DPU_QG_AMARELO), r.CD_POSTO_CN, r.FALHA_PARETO))
    conn.executemany("INSERT INTO dados (upload_id, nr_wo, dt, dpu, posto, falha) VALUES (?, ?, ?, ?, ?, ?)", rows)
    conn.commit()

def delete_period(conn, posto, year, start_date, end_date):
    conn.execute("DELETE FROM dados WHERE posto=? AND strftime('%Y', dt)=? AND datetime(dt) BETWEEN datetime(?) AND datetime(?)", (posto, str(year), datetime.combine(start_date, time(0,0,0)).isoformat(sep=" "), datetime.combine(end_date, time(23,59,59)).isoformat(sep=" ")))
    conn.commit()
...
def annual_end(df, ano):
    ...
    return max_d if max_d >= date(ano, 12, YEAR_CLOSE_DAY) else None
...
def svg_label(txt, max_len=18):
    txt = str(txt)
    return txt[:max_len-1] + "…" if len(txt) > max_len else txt

def plot_pareto_svg(p, posto, ano, ini, fim):
    if p.empty:
    ...
    max_q = max(float(p["Quantidade"].max()), 1.0)
    n = len(p)
    slot = pw / n
    bar_w = slot * 0.45
    def y_qty(v): return top + ph - (float(v)/max_q) * ph * 0.88
    def y_pct(v): return top + ph - (float(v)/100.0) * ph
    grid, bars, labels, points, markers = [], [], [], [], []
    for pct in range(0, 101, 20):
        y = y_pct(pct)
        grid.append(f'<line x1="{left}" y1="{y:.1f}" x2="{w-right}" y2="{y:.1f}" stroke="#d1d5db" stroke-width="1"/>')
        grid.append(f'<text x="{w-right+10}" y="{y+4:.1f}" font-size="13" fill="#111827">{pct}%</text>')
    for i, row in p.iterrows():
        x = left + slot*i + slot/2
        yb = y_qty(row["Quantidade"])
        bh = top + ph - yb
        bars.append(f'<rect x="{x-bar_w/2:.1f}" y="{yb:.1f}" width="{bar_w:.1f}" height="{bh:.1f}" fill="#2f62b3"/>')
        bars.append(f'<text x="{x:.1f}" y="{yb-8:.1f}" font-size="13" fill="#111827" text-anchor="middle">{int(row["Quantidade"])}</text>')
        label = html.escape(svg_label(row["Falha"], 22))
        labels.append(f'<g transform="translate({x-5:.1f},{top+ph+22:.1f}) rotate(-45)"><text font-size="13" fill="#111827" text-anchor="end">{label}</text></g>')
        yp = y_pct(row["% Acumulado"])
        points.append((x, yp, row["% Acumulado"]))
    polyline = " ".join([f"{x:.1f},{y:.1f}" for x,y,_ in points])
    for x,y,pct in points:
        markers.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="5" fill="#f97316"/>')
        markers.append(f'<text x="{x+8:.1f}" y="{y-8:.1f}" font-size="13" fill="#111827">{pct:.0f}%</text>')
    title = html.escape(f"Pareto de Falhas - Top 10 | {posto} | {ano} | {ini.strftime('%d/%m/%Y')} a {fim.strftime('%d/%m/%Y')}")
    svg = f'''
<div style="background:white;border-radius:12px;padding:12px;overflow-x:auto;">
<svg width="100%" viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="{w}" height="{h}" fill="white"/>
  <text x="{w/2}" y="30" text-anchor="middle" font-size="20" font-weight="700" fill="#111827">{title}</text>
  {''.join(grid)}
  <line x1="{left}" y1="{top}" x2="{left}" y2="{top+ph}" stroke="#111827"/>
  <line x1="{left}" y1="{top+ph}" x2="{w-right}" y2="{top+ph}" stroke="#111827"/>
  <line x1="{w-right}" y1="{top}" x2="{w-right}" y2="{top+ph}" stroke="#111827"/>
  <text x="25" y="{top+ph/2}" font-size="15" fill="#111827" transform="rotate(-90 25,{top+ph/2})" text-anchor="middle">Quantidade</text>
  <text x="{w-20}" y="{top+ph/2}" font-size="15" fill="#111827" transform="rotate(90 {w-20},{top+ph/2})" text-anchor="middle">% Acumulado</text>
  {''.join(bars)}
  <polyline points="{polyline}" fill="none" stroke="#111827" stroke-width="3"/>
  {''.join(markers)}
  {''.join(labels)}
  <rect x="390" y="{h-35}" width="22" height="12" fill="#2f62b3"/><text x="420" y="{h-24}" font-size="14" fill="#111827">Quantidade</text>
  <line x1="535" y1="{h-28}" x2="565" y2="{h-28}" stroke="#111827" stroke-width="3"/><circle cx="550" cy="{h-28}" r="5" fill="#f97316"/><text x="575" y="{h-24}" font-size="14" fill="#111827">% Acumulado</text>
</svg>
</div>
'''
...
con = get_conn()
init_db(con)
st.markdown('<div class="hero"><div style="font-size:1.75rem;font-weight:900;">RFT Automático - V9.9.7</div><div class="small">Sem Plotly/Matplotlib: Pareto clássico em SVG, upload com mapeamento de colunas e modos Diário/Semanal/Mensal/Anual.</div></div>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if df.empty:
        ...
        min_d, max_d = df["dt"].dt.date.min(), df["dt"].dt.date.max()
        st.caption(f"Janela consolidada: {min_d.strftime('%d/%m/%Y')} até {max_d.strftime('%d/%m/%Y')}")
        if modo == "Diário":
            start = end = st.date_input("Calendário - Dia", value=max_d, min_value=min_d, max_value=max_d, format="DD/MM/YYYY")
            label = start.strftime("%d/%m/%Y")
        elif modo == "Semanal":
            opts = week_options(df); labels = [x[0] for x in opts]
            chosen = st.selectbox("Semana", labels, index=len(labels)-1)
            _, start, end = opts[labels.index(chosen)]; label = chosen
        elif modo == "Mensal":
            opts = month_options(df, ano); labels = [x[0] for x in opts]
            chosen = st.selectbox("Mês", labels, index=len(labels)-1)
            _, start, end = opts[labels.index(chosen)]; label = chosen
        else:
            start = date(ano, 1, 1)
            ae = annual_end(df, ano)
            end = ae if ae else max_d
            label = f"Anual até {end.strftime('%d/%m/%Y')}" if ae else "Anual ainda não fechado"
        daily = calc_rft(df, max_d, max_d)
        if modo == "Diário":
            ws = start - timedelta(days=start.weekday()); we = ws + timedelta(days=6)
            weekly = calc_rft(df, ws, we)
            monthly = calc_rft(df, date(ano, start.month, 1), date(ano, start.month, monthrange(ano, start.month)[1]))
        else:
            weekly = calc_rft(df, start, end); monthly = calc_rft(df, start, end)
        ae = annual_end(df, ano)
        annual = calc_rft(df, date(ano, 1, 1), ae) if ae else (None, 0, 0, 0)
        ytd = calc_rft(df, date(ano, 1, 1), end)
...
with tabs[1]:
    ...
    if not anos:
        ...
        c1,c2 = st.columns(2)
        with c1: p_ano = st.selectbox("Ano do Pareto", anos, index=len(anos)-1)
        with c2: p_posto = st.radio("Posto do Pareto", POSTOS, horizontal=True)
        ...
        if dfp.empty:
            ...
            min_d, max_d = dfp["dt"].dt.date.min(), dfp["dt"].dt.date.max()
            d1,d2 = st.columns(2)
            with d1: data_ini = st.date_input("Data inicial", value=min_d, min_value=min_d, max_value=max_d, format="DD/MM/YYYY")
            with d2: data_fim = st.date_input("Data final", value=max_d, min_value=min_d, max_value=max_d, format="DD/MM/YYYY")
            if data_ini > data_fim:
                st.error("A data inicial não pode ser maior que a data final.")
            else:
                filt = dfp[(dfp["dt"].dt.date >= data_ini) & (dfp["dt"].dt.date <= data_fim)].copy()
                p = pareto(filt)
                plot_pareto_svg(p, p_posto, p_ano, data_ini, data_fim)
                if not p.empty:
                    show = p.copy()
...
with tabs[2]:
    ...
    st.warning("Mapeie as colunas da planilha. Não precisa que os nomes sejam exatamente iguais aos nomes técnicos.")
    ...
    if up is not None:
        try:
            raw = read_file(up); cols = list(raw.columns)
            mapping = {}; map_cols = st.columns(4)
            ...
            if falha_col: st.info(f"Coluna de falha usada: {falha_col}")
            else: st.warning("Nenhuma coluna de falha selecionada. O RFT será salvo, mas o Pareto ficará vazio.")
            preview = prepared.groupby([prepared["DT_HR_INSPECAO"].dt.year, "CD_POSTO_CN"]).agg(Linhas=("NR_WO", "count"), Falhas_preenchidas=("FALHA_PARETO", lambda s: (s != "").sum()), Data_min=("DT_HR_INSPECAO", "min"), Data_max=("DT_HR_INSPECAO", "max")).reset_index()
    ...
    if st.button("Salvar arquivo localmente", type="primary", use_container_width=True):
        if up is None or prepared is None or prepared.empty:
            ...
            uid = create_upload(con, up.name, len(prepared), message="Upload V9.9.7")
...
with tabs[4]:
    st.subheader("Sobre")
    st.write("V9.9.7: remove dependências de Plotly/Matplotlib, usa Pareto clássico em SVG e mantém Dashboard com Diário/Semanal/Mensal/Anual.")
