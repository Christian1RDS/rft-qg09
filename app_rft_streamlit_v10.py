...
st.set_page_config(page_title="RFT Automático - V10", page_icon="R", layout="wide")

DB = "rft_v10_local.db"
POSTOS = ["QG09", "QG07"]
CANON = ["NR_WO", "DT_HR_INSPECAO", "C_DPU_QG_AMARELO", "CD_POSTO_CN"]
ALIASES = {
    "NR_WO": ["NR_WO", "WO", "ORDEM", "ORDEM_PRODUCAO", "ORDEM DE PRODUCAO", "ORDEM DE PRODUÇÃO", "OP"],
    "DT_HR_INSPECAO": ["DT_HR_INSPECAO", "DATA", "DATA_INSPECAO", "DATA INSPECAO", "DATA INSPEÇÃO", "DATA_HORA", "DT_HR"],
...
html, body, [data-testid='stAppViewContainer'], .stApp { background: radial-gradient(circle at top left,#13213d 0%,#0b1220 38%,#09101c 100%); color:var(--txt); }
[data-testid='stHeader'] { background: rgba(11,18,32,.76); border-bottom: 1px solid var(--line); }
[data-testid='stSidebar'] { background:#0f172a; border-right: 1px solid var(--line); }
[data-testid='stSidebar'] * { color: var(--txt) !important; }
h1,h2,h3,h4,h5,h6,p,label,div,span { color: var(--txt); }
.hero { background:linear-gradient(135deg,rgba(24,34,53,.97),rgba(16,24,40,.98));border:1px solid var(--line);border-radius:22px;padding:1rem 1.25rem;margin-bottom:1rem;box-shadow:0 12px 36px rgba(0,0,0,.24); }
...
.kv { display:flex;justify-content:space-between;gap:1rem;padding:.55rem .7rem;border-radius:12px;background:rgba(255,255,255,.035);border:1px solid rgba(148,163,184,.10);margin-bottom:.45rem; }
...
def load(c, posto=None, ano=None):
    q = "SELECT nr_wo,dt,dpu,posto,COALESCE(falha,'') falha, upload_id FROM dados WHERE posto IN ('QG09','QG07')"
    p = []
    if posto:
        q += " AND posto=?"
        p.append(posto)
...
def latest_upload_info(c, df=None):
    if df is not None and not df.empty and 'upload_id' in df.columns:
        uid = int(df['upload_id'].max())
        out = pd.read_sql_query("SELECT id,file_name,uploaded_at,rows,message FROM uploads WHERE id=?", c, params=[uid])
    else:
        out = pd.read_sql_query("SELECT id,file_name,uploaded_at,rows,message FROM uploads ORDER BY id DESC LIMIT 1", c)
    return None if out.empty else out.iloc[0].to_dict()

def save_upload(c, name, df):
    cur = c.execute("INSERT INTO uploads (file_name,uploaded_at,rows,message) VALUES (?,?,?,?)", (name, datetime.now().isoformat(timespec='seconds'), len(df), 'Upload V10'))
    ...
    c.executemany("INSERT INTO dados (upload_id,nr_wo,dt,dpu,posto,falha) VALUES (?,?,?,?,?,?)", rows)
    c.commit()

def delete_period(c, posto, ano, ini, fim):
    c.execute("DELETE FROM dados WHERE posto=? AND strftime('%Y',dt)=? AND datetime(dt) BETWEEN datetime(?) AND datetime(?)", (posto, str(ano), datetime.combine(ini, time(0,0)).isoformat(sep=' '), datetime.combine(fim, time(23,59,59)).isoformat(sep=' ')))
    c.commit()

def delete_year(c, posto, ano):
    c.execute("DELETE FROM dados WHERE posto=? AND strftime('%Y',dt)=?", (posto, str(ano)))
    c.commit()

# ---------------- Upload robusto ----------------
def clean(df):
    d = df.copy()
    d.columns = [str(x).replace('\ufeff','').replace('ÿþ','').replace('\x00','').strip() for x in d.columns]
    return d
...
def bad_df(df):
    return len(df.columns) == 1 and any(s in str(df.columns[0]) for s in ['\t',';',','])

def read_csv_robust(content):
    encs = []
    if content[:2] in (b'\xff\xfe', b'\xfe\xff') or content[:200].count(b'\x00') > 20:
        encs += ['utf-16', 'utf-16-le', 'utf-16-be']
    encs += ['utf-8-sig', 'latin1', 'utf-16']
    last = None
    for enc in encs:
        try:
            text = content.decode(enc, errors='replace').replace('\ufeff','').replace('\x00','')
            sep = sniff_sep(text)
...
def read_file(up):
    ...
    b = up.getvalue()
    if ext in ['xlsx','xls']:
        return clean(pd.read_excel(io.BytesIO(b), engine='openpyxl' if ext=='xlsx' else 'xlrd'))
    if ext == 'csv':
        return read_csv_robust(b)
...
def detect_falha(cols):
    ...
    for c in cols:
        u = str(c).upper()
        if 'FALHA' in u or 'DEFEITO' in u or 'ANOMALIA' in u:
            return c
    return None
...
def prep(raw, mapa, falha_col):
    ...
    if mask.any():
        out.loc[mask,'DT_HR_INSPECAO'] = pd.to_datetime(raw.loc[mask,mapa['DT_HR_INSPECAO']], errors='coerce', dayfirst=True)
...
def month_opts(df, ano):
    ...
    for m in sorted(df['dt'].dt.month.unique()):
        ...
        res.append((f"{s.strftime('%m/%Y')} - {s.strftime('%d/%m/%Y')} a {e.strftime('%d/%m/%Y')}", s, e))
    return res

def annual_end(df, ano):
    mx = df['dt'].dt.date.max()
    return mx if mx >= date(ano,12,10) else None

def day_history(df):
    rows = []
    for d in sorted(df['dt'].dt.date.unique()):
        pct,total,good,bad = calc_rft(df, d, d)
        rows.append({'Dia': d.strftime('%d/%m/%Y'), 'RFT': pct or 0})
    return pd.DataFrame(rows)

def monthly_trend(df, ano, meta):
    rows = []
    for m in sorted(df['dt'].dt.month.unique()):
        s = date(ano,int(m),1); e = date(ano,int(m),monthrange(ano,int(m))[1])
        pct,total,good,bad = calc_rft(df,s,e)
        rows.append({'Mês': s.strftime('%m/%Y'), 'RFT': pct or 0, 'Meta': meta})
    return pd.DataFrame(rows)

def weekly_trend(df):
    rows = []
    for label,s,e in week_opts(df):
        pct,total,good,bad = calc_rft(df,s,e)
        rows.append({'Semana': label.split(' - ')[0], 'RFT': pct or 0})
    return pd.DataFrame(rows)
...
def plot_svg(p, posto, ano, ini, fim):
    if p.empty:
        ...
        return
    w,h = 1100,640
    left,right,top,bottom = 75,75,70,165
    pw,ph = w-left-right,h-top-bottom
    maxq = max(float(p['Quantidade'].max()),1)
    n = len(p); slot = pw/n; bw = slot*.45
    ...
    for pct in range(0,101,20):
        y = yp(pct)
        parts.append(f"<line x1='{left}' y1='{y:.1f}' x2='{w-right}' y2='{y:.1f}' stroke='#d1d5db'/><text x='{w-right+10}' y='{y+4:.1f}' font-size='13' fill='#111827'>{pct}%</text>")
    pts=[]
    for i,row in p.iterrows():
        x = left+slot*i+slot/2; yb = yq(row['Quantidade']); bh = top+ph-yb
        lab = html.escape(str(row['Falha'])[:21]+'…' if len(str(row['Falha']))>22 else str(row['Falha']))
    ...
    for x,y,pct in pts:
        parts.append(f"<circle cx='{x:.1f}' cy='{y:.1f}' r='5' fill='#f97316'/><text x='{x+8:.1f}' y='{y-8:.1f}' font-size='13' fill='#111827'>{pct:.0f}%</text>")
...
def meta_panel(pct, meta):
    css = 'neutral' if pct is None else ('ok' if pct >= meta else 'bad')
    val = fmt(pct)
    diff = 'Sem dados' if pct is None else f'{pct-meta:+.2f} p.p.'.replace('.',',')
    st.markdown(f"""
    <div class='panel'>
      <div style='font-size:1.08rem;font-weight:800;'>Meta x resultado</div>
      <div class='small'>Regra visual aplicada conforme sua configuracao.</div>
      <div class='{css}' style='font-size:2.15rem;font-weight:900;margin:.5rem 0;'>{val}</div>
      <div class='small'>Meta: <b>{str(meta).replace('.', ',')}%</b><br>Diferenca: <b>{diff}</b><br>Regra: abaixo da meta = vermelho | acima ou igual a meta = verde</div>
    </div>
    """, unsafe_allow_html=True)

def resumo_panel(info, posto, ano, min_d, max_d, recorte, meta):
    file_name = '-' if info is None else info.get('file_name','-')
    uploaded_at = '-' if info is None else info.get('uploaded_at','-')
    rows = [('Recorte',recorte),('Ultimo arquivo salvo',file_name),('Ultimo upload',uploaded_at),('Posto',posto),('Ano',str(ano)),('Janela consolidada',f"{min_d.strftime('%d/%m/%Y')} ate {max_d.strftime('%d/%m/%Y')}"),('Meta ativa',str(meta).replace('.',',')+'%')]
...
# ---------------- App ----------------
c = con()
st.markdown("<div class='hero'><div style='font-size:1.75rem;font-weight:900;'>RFT Automático - V10</div><div class='small'>Retorno da aba Tendência, Meta x resultado, Resumo executivo e Leitura diária do RFT. CSV tabulado/UTF-16 corrigido.</div></div>", unsafe_allow_html=True)
tabs = st.tabs(['Dashboard','Tendência','Pareto de Falhas','Base & Upload','Histórico','Sobre'])

with st.sidebar:
    posto = st.radio('Posto', POSTOS, horizontal=True)
    anos_p = years(c, posto)
    ano = st.selectbox('Ano', anos_p if anos_p else [date.today().year], index=len(anos_p)-1 if anos_p else 0)
    modo = st.radio('Modo', ['Diário','Semanal','Mensal','Anual'])
    meta = st.number_input('Meta RFT (%)', 0.0, 100.0, 95.0, 0.1)

with tabs[0]:
    st.subheader('Dashboard')
    df = load(c, posto, ano) if anos_p else pd.DataFrame()
    if df.empty:
        st.info('Sem dados para o posto/ano selecionado.')
    else:
        mn,mx = df['dt'].dt.date.min(), df['dt'].dt.date.max()
        if modo=='Diário':
            start=end=st.date_input('Calendário - Dia', value=mx, min_value=mn, max_value=mx, format='DD/MM/YYYY'); recorte=start.strftime('%d/%m/%Y')
        elif modo=='Semanal':
            opts=week_opts(df); labs=[x[0] for x in opts]; ch=st.selectbox('Semana', labs, index=len(labs)-1); _,start,end=opts[labs.index(ch)]; recorte=ch
        elif modo=='Mensal':
            opts=month_opts(df,ano); labs=[x[0] for x in opts]; ch=st.selectbox('Mês', labs, index=len(labs)-1); _,start,end=opts[labs.index(ch)]; recorte=ch
        else:
            start=date(ano,1,1); ae=annual_end(df,ano); end=ae if ae else mx; recorte=f"Anual até {end.strftime('%d/%m/%Y')}" if ae else 'Anual ainda não fechado'
        daily=calc_rft(df,mx,mx)
        if modo=='Diário':
            ws=start-timedelta(days=start.weekday()); we=ws+timedelta(days=6); weekly=calc_rft(df,ws,we); monthly=calc_rft(df,date(ano,start.month,1),date(ano,start.month,monthrange(ano,start.month)[1]))
        else:
            weekly=calc_rft(df,start,end); monthly=calc_rft(df,start,end)
        ...
        for col,(name,val) in zip(st.columns(5),[('RFT Diário',daily),('RFT Semanal',weekly),('RFT Mensal',monthly),('RFT Anual',annual),('RFT YTD',ytd)]):
            col.metric(name,fmt(val[0]),f'Total: {val[1]}')
...
with tabs[1]:
    ...
    if df.empty:
        ...
        c1,c2 = st.columns(2)
        with c1:
            st.markdown("<div class='panel'><div style='font-size:1.08rem;font-weight:800;'>Tendência mensal</div>", unsafe_allow_html=True)
            mt = monthly_trend(df, ano, meta)
            if not mt.empty:
                st.bar_chart(mt.set_index('Mês')[['RFT','Meta']], use_container_width=True)
                st.dataframe(mt, use_container_width=True, hide_index=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with c2:
            st.markdown("<div class='panel'><div style='font-size:1.08rem;font-weight:800;'>Tendência semanal</div>", unsafe_allow_html=True)
            wt = weekly_trend(df)
            if not wt.empty:
...
with tabs[3]:
    ...
    st.warning('V10 mantém leitura robusta de CSV tabulado/UTF-16. Mapeie as colunas abaixo.')
...
with tabs[4]:
    ...
    h=uploads(c)
    if h.empty: st.info('Sem uploads salvos.')
    else: st.dataframe(h,use_container_width=True,hide_index=True)

with tabs[5]:
    st.subheader('Sobre')
    st.write('V10: retorno da aba Tendência, Meta x resultado, Resumo executivo do recorte e gráfico de Leitura diária do RFT. Mantém Pareto SVG e leitura robusta de CSV tabulado/UTF-16.')
