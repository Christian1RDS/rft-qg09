...
st.set_page_config(page_title="RFT Pareto - V9.9.5", page_icon="R", layout="wide")
...
def db():
    ...
    con.execute("CREATE TABLE IF NOT EXISTS uploads (id INTEGER PRIMARY KEY AUTOINCREMENT, file_name TEXT, uploaded_at TEXT, rows INTEGER)")
    con.execute("CREATE TABLE IF NOT EXISTS dados (id INTEGER PRIMARY KEY AUTOINCREMENT, upload_id INTEGER, nr_wo TEXT, dt TEXT, dpu REAL, posto TEXT, falha TEXT)")
    con.commit()
    return con
...
def read_file(f):
    ...
    b = f.getvalue()
    if ext in ["xlsx", "xls"]:
        return pd.read_excel(io.BytesIO(b), engine="openpyxl" if ext == "xlsx" else "xlrd")
    if ext == "csv":
        last = None
        for enc in ["utf-8-sig", "latin1", "utf-16"]:
            for sep in [None, ";", ",", "	"]:
                try:
                    if sep is None:
                        return pd.read_csv(io.BytesIO(b), encoding=enc, sep=None, engine="python")
                    return pd.read_csv(io.BytesIO(b), encoding=enc, sep=sep)
...
def prepare(df, falha_col=None):
    ...
    if miss:
        raise ValueError("Colunas obrigatórias ausentes: " + ", ".join(miss))
    out = df.copy()
    out["DT_HR_INSPECAO"] = pd.to_datetime(out["DT_HR_INSPECAO"], errors="coerce")
    ...
    col = falha_col or detect_falha_col(out)
    out["FALHA_PARETO"] = out[col].fillna("").astype(str).str.strip() if col else ""
    out = out[out["CD_POSTO_CN"].isin(POSTOS) & out["DT_HR_INSPECAO"].notna()].copy()
    return out, col

def save_upload(con, name, df):
    cur = con.execute("INSERT INTO uploads (file_name, uploaded_at, rows) VALUES (?, ?, ?)", (name, datetime.now().isoformat(timespec="seconds"), len(df)))
    uid = int(cur.lastrowid)
    rows = [(uid, r.NR_WO, r.DT_HR_INSPECAO.isoformat(sep=" ", timespec="seconds"), float(r.C_DPU_QG_AMARELO), r.CD_POSTO_CN, r.FALHA_PARETO) for r in df.itertuples(index=False)]
    con.executemany("INSERT INTO dados (upload_id, nr_wo, dt, dpu, posto, falha) VALUES (?, ?, ?, ?, ?, ?)", rows)
    con.commit()

def load(con, posto=None, ano=None):
...
def years(con):
    df = pd.read_sql_query("SELECT DISTINCT CAST(strftime('%Y', dt) AS INT) AS ano FROM dados ORDER BY ano", con)
...
def plot_pareto(p, posto, ano, ini, fim):
    ...
    fig.add_trace(go.Bar(x=p["Falha"], y=p["Quantidade"], name="Quantidade", text=p["Quantidade"], textposition="outside", marker_color="#2f62b3"), secondary_y=False)
    fig.add_trace(go.Scatter(x=p["Falha"], y=p["% Acumulado"], name="% Acumulado", mode="lines+markers+text", text=[f"{v:.0f}%" for v in p["% Acumulado"]], textposition="top center", line=dict(color="#f97316", width=3), marker=dict(size=8)), secondary_y=True)
    fig.add_hline(y=80, line_dash="dash", line_color="#ef4444", annotation_text="80%", annotation_position="top right", secondary_y=True)
    fig.update_yaxes(title_text="Quantidade", secondary_y=False, range=[0, max(p["Quantidade"].max() * 1.25, 1)])
    ...
    fig.update_layout(title=dict(text=f"Pareto de Falhas - Top 10 | {posto} | {ano} | {ini.strftime('%d/%m/%Y')} a {fim.strftime('%d/%m/%Y')}", x=.5), height=650, margin=dict(l=65, r=65, t=85, b=185), legend=dict(orientation="h", y=-.35, x=.5, xanchor="center"), plot_bgcolor="white", paper_bgcolor="white", font=dict(color="#111827"), bargap=.35)
    st.plotly_chart(fig, use_container_width=True)

con = db()
st.title("RFT Automático - V9.9.5")
st.caption("Pareto clássico com Plotly: barras + curva acumulada, Top 10 e filtro de período.")
...
with tabs[0]:
    ...
    df = load(con, posto, ano) if anos else pd.DataFrame()
    if df.empty:
        st.info("Sem dados para o posto/ano selecionado.")
    else:
        ini = df["dt"].dt.date.min()
        fim = df["dt"].dt.date.max()
        pct, total, good, bad = rft(df, ini, fim)
        st.metric("RFT YTD", "Sem dados" if pct is None else f"{pct:.2f}%".replace(".", ","), f"Total: {total} | Boas: {good} | Ruins: {bad}")
