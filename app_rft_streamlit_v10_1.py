...
st.set_page_config(page_title="RFT Automático - V10.1", page_icon="R", layout="wide")
DB="rft_v101_local.db"
POSTOS=["QG09","QG07"]
...
ALIASES={
 ...
 "CD_MODELO":["CD_MODELO","MODELO","MODEL","PRODUTO","COD_MODELO"]
}
...
[data-testid='stSidebar']{background:#0f172a;} [data-testid='stSidebar'] *{color:var(--txt)!important;}
...
def con():
    ...
    if "modelo" not in cols:
        c.execute("ALTER TABLE dados ADD COLUMN modelo TEXT")
    c.commit(); return c
...
def load(c, posto=None, ano=None):
    ...
    if not df.empty:
        df['dt']=pd.to_datetime(df['dt'],errors='coerce'); df['dpu']=pd.to_numeric(df['dpu'],errors='coerce').fillna(0); df['falha']=df['falha'].fillna('').astype(str).str.strip(); df['modelo']=df['modelo'].fillna('').astype(str).str.strip(); df=df[df['dt'].notna()].copy()
    return df
...
def latest_upload_info(c, df=None):
    if df is not None and not df.empty and 'upload_id' in df.columns:
        uid=int(df['upload_id'].max()); out=pd.read_sql_query("SELECT id,file_name,uploaded_at,rows,message FROM uploads WHERE id=?",c,params=[uid])
...
def save_upload(c,name,df):
    cur=c.execute("INSERT INTO uploads (file_name,uploaded_at,rows,message) VALUES (?,?,?,?)",(name,datetime.now().isoformat(timespec='seconds'),len(df),'Upload V10.1'))
...
def prep(raw,mapa,falha_col,modelo_col):
    ...
    out['CD_MODELO']=raw[modelo_col].fillna('').astype(str).str.strip() if modelo_col else 'Sem modelo'
...
def day_history(df):
    return pd.DataFrame([{'Dia':d.strftime('%d/%m/%Y'),'RFT':calc_rft(df,d,d)[0] or 0} for d in sorted(df['dt'].dt.date.unique())])
...
def pareto(df,top_n=10):
    ...
    p=f['falha'].value_counts(sort=True,ascending=False).head(int(top_n)).reset_index(); p.columns=['Falha','Quantidade']; total=p['Quantidade'].sum(); p['%']=(p['Quantidade']/total*100).round(2); p['% Acumulado']=p['%'].cumsum().round(2); p.insert(0,'Rank',range(1,len(p)+1)); return p
...
# ---------------- App ----------------
c=con(); st.markdown("<div class='hero'><div style='font-size:1.75rem;font-weight:900;'>RFT Automático - V10.1</div><div class='small'>Correção do RFT Diário e estratificação do Top escolhido por modelo.</div></div>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    if df.empty: st.info('Sem dados para tendência.')
        ...
        a,b=st.columns(2)
        with a:
            mt=monthly_trend(df,ano,meta); st.markdown("<div class='panel'><b>Tendência mensal</b>",unsafe_allow_html=True); st.bar_chart(mt.set_index('Mês')[['RFT','Meta']],use_container_width=True); st.dataframe(mt,use_container_width=True,hide_index=True); st.markdown('</div>',unsafe_allow_html=True)
        with b:
            wt=weekly_trend(df); st.markdown("<div class='panel'><b>Tendência semanal</b>",unsafe_allow_html=True); st.bar_chart(wt.set_index('Semana'),use_container_width=True); st.dataframe(wt,use_container_width=True,hide_index=True); st.markdown('</div>',unsafe_allow_html=True)
with tabs[2]:
        ...
        if dfp.empty: st.info('Sem falhas preenchidas para o posto/ano selecionado.')
            ...
            if ini>fim: st.error('A data inicial não pode ser maior que a data final.')
                ...
                if not p.empty:
                    ...
                    escolha=st.selectbox('Escolha uma falha do Top para estratificar por modelo', p['Falha'].tolist())
                    detalhe=filt[filt['falha']==escolha].copy(); detalhe['modelo']=detalhe['modelo'].replace('', 'Sem modelo')
                    modelos=detalhe['modelo'].value_counts().head(20).reset_index(); modelos.columns=['Modelo','Quantidade']
                    st.markdown(f"### Estratificação por modelo — {escolha}")
                    if modelos.empty: st.info('Não há modelo preenchido para esta falha no período selecionado.')
                    else:
...
with tabs[3]:
    st.subheader('Base & Upload'); st.warning('Mapeie as colunas. CD_MODELO é opcional, mas necessário para estratificar o Pareto por modelo.')
...
with tabs[5]:
    st.subheader('Sobre'); st.write('V10.1: corrige o RFT Diário para usar o dia selecionado e adiciona escolha do Top do Pareto + estratificação da falha selecionada por modelo.')
